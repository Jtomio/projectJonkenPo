...
function jogar(escolha) {
    ...
    if(ganhador == 0) {
        mensagem('Ninguém ganhou essa, deu EMPATE!');
    }
    else if(ganhador == 1) {
        mensagem('O ponto vai para ' + jogadorNome);
        somarPontoJogador();
    }
    else if(ganhador = 2) {
        mensagem('O Computador ganhou essa.');
        somarPontoComputador();
    }

    // Tirar as opções selecionadas
    setTimeout(function() {
        deselecionar('jogador', jogadorEscolha);
        deselecionar('computador', computadorEscolha);
        mensagem(jogadorNome + ' escolha outra jogada.');
    }, 3500);
};
